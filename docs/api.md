...
Full bill detail.
`200` → `{ bill, images: [{ id, image_url, position }, ...], items: [{ id, name, price, unit_note, contributors: [{ user_id, name, email, share_amount }] }], extra_charges: [{ id, name, amount }], charges: [{ user_id, name, email, amount }] }`. `bill` includes `tip_amount`, `tip_paid_by`, and a joined `tip_paid_by_name` (`null` unless one member covered the tip themselves). `extra_charges` is the named list (e.g. `"Tax"`, `"B.C.H.Fee"`) as entered/scanned; `charges` is each contributor's equal share of `sum(extra_charges)` + any shared tip pooled together — empty when the bill had neither.
